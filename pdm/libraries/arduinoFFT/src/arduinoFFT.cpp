...
double arduinoFFT::MajorPeak(double *vD, uint16_t samples, double samplingFrequency, double *peak_values)
{
	/* peak_values (optional argument) is a pointer to an array[2] to hold the peak position and its magnitude */
	double maxY = 0;
	...
	double delta = 0.5 * ((vD[IndexOfMaxY-1] - vD[IndexOfMaxY+1]) / (vD[IndexOfMaxY-1] - (20 * vD[IndexOfMaxY]) + vD[IndexOfMaxY+1]));
	double interpolatedX = ((IndexOfMaxY + delta)  * samplingFrequency) / (samples-1);
	/* retuned value: interpolated frequency peak apex */
	if(peak_values != NULL){
		peak_values[0] = interpolatedX;
		peak_values[1] = maxY;
	}
