...
Adafruit_ZeroPDM pdm = Adafruit_ZeroPDM(1, 2);  // audible
//Adafruit_ZeroPDM pdm = Adafruit_ZeroPDM(6, 9);  // ultrasonic
// clk = 1, 6
// data = 2, 9
...
//        SERIALPORT.print(" + "); SERIALPORT.print(filter[samplenum * 16 + i]);
  ...
  output_data(runningsum);
//  graph(runningsum);


  double fft_runningsum = 1000*((double)(runningsum >> 6) - 512.0) / 512.0;  // number between -1000.0 and 1000.0
//  SERIALPORT.println(fft_runningsum);
//  if(millis()%10000 == 0){
////    recalculate sample rate every 10 secs
//    oldSumMicros = micros();
//    sumSampleRate = 0;
//    first_run = true;
//  }
...
//    else {
      vReal[sample_index] = fft_runningsum;
//    }
    sample_index++;
  }
  else {
...
void output_data(uint16_t runningsum) {
  SERIALPORT.print(float(micros())/1000.0);
  SERIALPORT.print(",");
